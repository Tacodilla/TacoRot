...
local function Fallback(forceSafe)
  if forceSafe or not A then
    return 403 or SAFE -- Lightning Bolt
  end
  return (Known(A.LightningBolt) and A.LightningBolt)
      or (Known(A.EarthShock) and A.EarthShock)
      or 403 or SAFE
end
...
local function BuildQueue()
  ...
  if tree == 1 then
    ...
    if A and stacks and stacks >= 5 and ReadySoon(A.LightningBolt) then Push(q, A.LightningBolt) end
  end
  return q
end
...
function TR:StartEngine_Shaman()
  ...
  if AceTimer and AceTimer.ScheduleRepeatingTimer then
    self._engineTimer_SH = AceTimer:ScheduleRepeatingTimer(function()
      _G.TacoRot:EngineTick_Shaman()
    end, 0.20)
  else
...
do
  ...
  if class == "SHAMAN" then TR:StartEngine_Shaman() end
end
