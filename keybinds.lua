...
local function ReadKeybindings()
  if not TR.IsValidSpec or not TR:IsValidSpec() then return end
...
function TR:GetKeybindForSpell(spellName, displayLower)
  ...
  local useConsole = false
  local useLower = displayLower or false

  if useConsole and bindData.console[1] then
    return bindData.console[1]
  elseif useLower and bindData.lower[1] then
...
frame:SetScript("OnEvent", function(self, event, ...)
  if event == "ADDON_LOADED" and (...) == "TacoRot" then
    TR:ScheduleTimer(ReadKeybindings, 1)
  elseif event == "PLAYER_LOGIN" then
    TR:ScheduleTimer(ReadKeybindings, 2)
  elseif event == "UPDATE_BINDINGS" or event == "ACTIONBAR_SLOT_CHANGED" or event == "SPELLS_CHANGED" then
    TR:ScheduleTimer(ReadKeybindings, 0.1)
  end
end)
